...
export function Hero() {
  ...
  return (
    <section id="inicio" className="relative grid min-h-[max(42rem,100svh)] overflow-hidden bg-cream lg:min-h-[max(720px,100svh)]" aria-labelledby="hero-title">
      ...
      <div className="relative z-10 mx-auto flex min-h-full w-[min(calc(100%-3rem),1430px)] items-start md:w-[min(calc(100%-6vw),1430px)] md:min-h-[74svh] md:items-start md:pl-[2vw] md:pt-[clamp(7rem,12svh,8rem)]">
        <motion.div className="w-[min(58%,16rem)] pt-[clamp(8.75rem,17svh,10.5rem)] md:w-full md:max-w-[35rem] md:pt-0" initial={reduceMotion ? false : { opacity: 0, y: 24 }} animate={reduceMotion ? {} : { opacity: 1, y: 0 }} transition={{ duration: .62, ease: 'easeOut' }}>
      ...
      <div className="relative z-10 mt-10 mb-8 ml-6 flex items-center gap-3 text-[.65rem] leading-tight text-ink md:absolute md:bottom-12 md:left-[5vw] md:mt-0 md:mb-0 md:ml-0 md:flex md:flex-wrap md:gap-3 md:text-[clamp(.72rem,.86vw,.88rem)]">
        <ShieldCheck className="shrink-0 text-gold" size={25} aria-hidden="true" /><span>Confidencialidad<span className="block md:hidden">Garantizada</span></span><b className="hidden text-gold md:inline" aria-hidden="true">•</b><span className="hidden md:inline">Experiencia</span><b className="hidden text-gold md:inline" aria-hidden="true">•</b><span className="hidden md:inline">Compromiso</span>
      </div>
    </section>
  )
}
